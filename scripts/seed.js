...
//
// Everything this loads is synthetic and was generated by the team. No real
// persons, cases or personal data are represented.
//
// Order matters: every evidence file is fingerprinted from its CURRENT bytes
// at seed time. The demo then alters one file afterwards, so that the stored
// fingerprint reflects the original and verification fails as intended.
//
...
// A fixed namespace makes seeding reproducible, so a demo reset produces the
// same ids every time and the tamper scripts keep working.
...
await tx(async (client) => {
  // Clear in dependency order. The append only trigger fires on DELETE, so
  // it is disabled for the seed transaction only. This is the seeder, not a
  // route: no request can reach this path.
  ...
  for (const c of data.cases) {
    ...
    for (const item of c.items) {
      ...
      // An offline item was sealed when the device regained network. Every
      // event recorded before that moment is stamped by the server at that
      // moment, which is what produces the visible gap in the report.
      ...
      // corrects_seq in the pack is a position within this item's chain.
      // Resolve it to a real event id now that every event exists. This does
      // not alter any hashed field, so the chain is unaffected.
      ...
      // One case level event per item creation, so an outright deletion of
      // the item still leaves a knot in the string.
